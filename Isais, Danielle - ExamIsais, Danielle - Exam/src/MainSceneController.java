...
public class MainSceneController {
    ...
    void calculateFare(ActionEvent event) {
        ...
        if (startStationOpt.isPresent() && endStationOpt.isPresent() && startStationOpt.get().equals(endStationOpt.get())) {
            errorLabel.setText("Start and end destinations cannot be the same.");
            return;
        } else {
            errorLabel.setText("");
        }
    ...
    private void validateInputs() {
        boolean validInputs = Optional.ofNullable(ComboStart.getValue())
                .flatMap(start -> Optional.ofNullable(ComboEnd.getValue())
                        .flatMap(end -> Optional.ofNullable(ComboDiscount.getValue())
                                .map(discount -> !start.equals(end))))
                .orElse(false);
        calculateButton.setDisable(!validInputs);
    }
